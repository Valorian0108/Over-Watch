...
function AssetRow({ asset, selected, onSelect }: { asset: MarketAsset; selected: boolean; onSelect: () => void }) {
  return (
    <button type="button" onClick={onSelect} className={`group grid w-full grid-cols-[auto_1fr_auto] items-center gap-3 rounded-2xl border px-3 py-3 text-left transition-all duration-300 ${selected ? 'border-[#6f7770] bg-[#e2ebd1] shadow-sm' : 'border-transparent hover:border-border hover:bg-muted/55'}`} data-testid={`button-select-asset-${asset.symbol.toLowerCase()}`} aria-pressed={selected}>
