...
async function cmcGet(path, query) {
...
app.get('/api/market/assets', async (req, res) => {
  try {
    const kind = req.query.kind === 'crypto' || req.query.kind === 'rwa' ? req.query.kind : undefined;
    const limit = parseInt(req.query.limit) || 8;
...
app.get('/api/market/assets/:symbol', async (req, res) => {
  try {
    ...
    console.error('Market asset error:', error);
    res.status(502).json({ error: error.message || 'Market data is unavailable.' });
  }
});
...
app.get('/api/market/search', async (req, res) => {
  try {
    ...
    // Simple search implementation - return empty for now
    res.json([]);
  } catch (error) {
...
app.post('/api/market/explain', async (req, res) => {
  try {
    const { question, assetSymbol } = req.body;
    ...
    console.error('Market explain error:', error);
    res.status(502).json({ error: error.message || 'Market data is unavailable.' });
  }
});
...
if (process.env.NODE_ENV !== 'vercel') {
  app.listen(PORT, () => {
    console.log(`Tidepool API server running on port ${PORT}`);
  });
}
